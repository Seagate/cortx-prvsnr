...
# Note. primary server node salt service restart is in foreground,
# so minion will reported to restarted salt service on primary server node
salt_master_service_restarted:
  cmd.run:
    # 1. test.true will prevent restart of salt service on primary server node if the config is malformed
