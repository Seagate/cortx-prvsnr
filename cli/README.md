...
Installs the provisioner repository along with SaltStack on the hosts with bare OS installed, configures salt primary/minion connections and makes some additional preliminary configuration.
...
- `--salt-master=HOSTNAME` the hostname/IP to use to configure salt minions connections to primary.
...
Configure cluster with a primary on srvnode-1
