...
Step 2: Verify the services with corresponding ports, are mapped to all the zones with the below command.
```bash
firewall-cmd --list-all
```

Step 3: Verify if the services are mapped to the zones on all the nodes.
```bash
ssh srvnode-2 firewall-cmd --list-all
ssh srvnode-3 firewall-cmd --list-all
```
