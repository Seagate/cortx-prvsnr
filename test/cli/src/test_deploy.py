...
def test_deploy_cortx_commands(
    ...
    if singlenode:
        expected_lines = [
            'SALT-ARGS: srvnode_1 state.apply components.{}'.format(state)
            for state in [
                'system',
                'ha.haproxy',
                'misc_pkgs.openldap',
        ...
        expected_lines = [
            'SALT-ARGS: srvnode_[1,2] state.apply components.{}'.format(state)
            for state in ['system', 'ha.haproxy', 'misc_pkgs.openldap']
        ] + [
            'SALT-ARGS: srvnode_1 state.apply components.misc_pkgs.build_ssl_cert_rpms',  # noqa: E501
            'SALT-ARGS: srvnode_2 state.apply components.misc_pkgs.build_ssl_cert_rpms'  # noqa: E501
        ] + [
            'SALT-ARGS: srvnode_[1,2] state.apply components.{}'.format(state)
