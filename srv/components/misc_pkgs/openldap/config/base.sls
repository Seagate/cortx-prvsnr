...
Configure OpenLDAP - Schema:
  cmd.run:
    ...
    - env:
      - password: {{ ldap_password }}
    # - unless:
    #   - ldapsearch -x -w {{ salt['lyveutil.decrypt']('openldap', pillar['openldap']['root']['secret']) }} -D "cn=admin,dc=seagate,dc=com" -H ldap:// -b "cn={1}s3user,cn=schema,cn=config"
    - require:
      - Remove existing file
    - watch_in:
      - Restart slapd service
...
Configure OpenLDAP - Base data:
  cmd.run:
    ...
    #   - ldapsearch -x -w {{ salt['lyveutil.decrypt']('openldap', pillar['openldap']['root']['secret']) }} -D "cn=admin,dc=seagate,dc=com" -H ldap:// -b "ou=accounts,dc=s3,dc=seagate,dc=com"
    #   - ldapsearch -x -w {{ salt['lyveutil.decrypt']('openldap', pillar['openldap']['root']['secret']) }} -D "cn=admin,dc=seagate,dc=com" -H ldap:// -b "ou=accesskeys,dc=s3,dc=seagate,dc=com"
    #   - ldapsearch -x -w {{ salt['lyveutil.decrypt']('openldap', pillar['openldap']['root']['secret']) }} -D "cn=admin,dc=seagate,dc=com" -H ldap:// -b "ou=idp,dc=s3,dc=seagate,dc=com"
    - env:
      - password: {{ ldap_password }}
    - require:
      - Configure OpenLDAP - Schema
...
Configure OpenLDAP - Add IAM admin:
  cmd.run:
    ...
    - env:
      - password: {{ ldap_password }}
    # - unless:
    #   - ldapsearch -x -w {{ salt['lyveutil.decrypt']('openldap', pillar['openldap']['root']['secret']) }} -D "cn=admin,dc=seagate,dc=com" -H ldap:// -b "cn=sgiamadmin,dc=seagate,dc=com"
    - require:
      - Configure OpenLDAP - Base data
    - watch_in:
      - Restart slapd service
...
Configure OpenLDAP - Enable IAM constraints:
  cmd.run:
    ...
    - env:
      - password: {{ ldap_password }}
    # - unless:
    #   - ldapsearch -x -w {{ salt['lyveutil.decrypt']('openldap', pillar['openldap']['root']['secret']) }} -D "cn=admin,dc=seagate,dc=com" -H ldap:// -b "cn=module{0},cn=config"
    #   - ldapsearch -x -w {{ salt['lyveutil.decrypt']('openldap', pillar['openldap']['root']['secret']) }} -D "cn=admin,dc=seagate,dc=com" -H ldap:// -b "olcOverlay=unique,olcDatabase={2}{{ pillar['openldap']['backend_db'] }},cn=config"
    - require:
      - Configure OpenLDAP - Setup permissions for IAM admin
    - watch_in:
      - Restart slapd service
