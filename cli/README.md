...
2. `configure-cortx`: adjusts pillars for provisioner components
3. `deploy-cortx`: installs CORTX stack using Salt
4. `bootstrap-cortx`: initializes CORTX services
5. `start-cortx`: starts/restarts CORTX services
6. `stop-cortx`: stops CORTX services
...
```shell
$ deploy-cortx -r srvnode-1 -F ./ssh_config
```

Install CORTX stack for a single node with a remote host as srvnode-1

```shell
$ deploy-cortx -r srvnode-1 -F ./ssh_config --singlenode
```
...
```shell
$ start-cortx --remote srvnode-1 -F ./ssh_config
```
...
```shell
$ start-cortx --remote srvnode-1 -F ./ssh_config --restart
```
...
```shell
$ stop-cortx --remote srvnode-1 -F ./ssh_config
```
...
2. `configure-cortx -p cluster >./cluster.sls`
3. ... edit `cluster.sls` manually ...
4. `configure-cortx -f ./cluster.sls cluster`
5. `configure-cortx -p release >./release.sls`
6. ... edit `release.sls` manually ...
7. `configure-cortx -f ./release.sls release`
8. `deploy-cortx -S`
9. `bootstrap-cortx -S`
10. `start-cortx -S`
